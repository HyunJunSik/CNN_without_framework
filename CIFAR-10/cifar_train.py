...
PATH = os.path.join(PATH, 'data', 'cifar_train.npz')

def train_data_loader(file):
    dataset = np.load(file)
    data = dataset['data'][20000:, :]
    labels = dataset['labels'][20000:].reshape(len(data), 1)
...
train_data, train_label = train_data_loader(PATH)
...
# PATH = os.path.dirname(__file__)
# PATH = os.path.join(PATH, 'cifar_params.pkl')
...
for epoch in range(num_epochs):
    ...
    print("epoch :", epoch)
    cnt = 1
    for b in range(0, len(train_label), batch_size):
        batchs = dataset[b : b + batch_size]
        params, cost = Adam(batchs, num_classes, learning_rate, 32, image_channel, beta1, beta2, params, cost)
        print("batch", cnt, "/", int(len(train_data) / batch_size))
        cnt += 1

to_save = params

with open("cifar_params2.pkl", 'wb') as file:
